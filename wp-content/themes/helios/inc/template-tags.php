...
if (! function_exists('cenote_posted_on')) :
	...
	function cenote_posted_on($display = true)
	{
		...
		$posted_on = sprintf(
			'%s',
			'<a class="posted-on-link" href="' . esc_url(get_permalink()) . '" rel="bookmark">' . $time_string . '</a>'
		);
		if ($display == true) {
...
if (! function_exists('cenote_post_thumbnail')) {
	...
	function cenote_post_thumbnail($display = true, $thumbnail_size = 'post-thumbnail')
	{
		...
		$entryThumbnail = '';
		if (is_singular()) {
			$entryThumbnail .= '<div class="entry-thumbnail">';
			if ('layout--no-sidebar' === $layout_style) {
				$entryThumbnail .= get_the_post_thumbnail(null, 'cenote-full-width');
			} else {
				$entryThumbnail .= get_the_post_thumbnail();
			}
			$entryThumbnail .= '</div><!-- .post-thumbnail -->';
		} else {

			$entryThumbnail .= '<a class="entry-thumbnail" href="';
			$entryThumbnail .= get_the_permalink();
			$entryThumbnail .= '" aria-hidden="true">';
			//			if ( ! empty( $title_first_letter ) ){
			//				$entryThumbnail .= '<span class="post-thumbnail__letter">';
			//				$entryThumbnail .= esc_html( $title_first_letter );
			//			}
			$entryThumbnail .=
...
function cenote_post_categories($display = true)
{
	...
	if ('post' === get_post_type()) {
		...
		if ($categories_list) {
			/* translators: 1: list of categories. */
			$cats = sprintf('<span class="cat-links">' . '%1$s' . '</span>', $categories_list); // phpcs:ignore
		}
	}
	if ($display == true) {
		echo $cats;
