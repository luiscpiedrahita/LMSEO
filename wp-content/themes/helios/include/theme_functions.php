...
add_image_size('Blog', 1150, 770, TRUE);
add_image_size('Home', 1890, 910, TRUE);
...
function zp_portfolio_category_filter() {
	...
	$output .=  '<ul id="portfolio-categories" class="option-set" data-option-key="filter"><li><a href="#" data-option-value="*" class="selected">' . __('show all', 'novo') . '</a></li>';
...
function zp_portfolio_items_term($id) {
	...
	$terms = wp_get_post_terms($id, 'portfolio-category');
	$term_string = '';
...
function zp_portfolio_style_effects($style) {
	...
	switch ($style) {
		case 'fall':
			$class = 'zp-effect-fall zp-effect-delay';
			break;
		case 'slide':
			$class = 'zp-effect-slide';
			break;
		case 'rotate_fall':
			$class = 'zp-stacking-inversed zp-effect-fallrotate zp-effect-delay-reversed';
			break;
		case 'rotate_scale':
			$class = 'zp-effect-scalerotate zp-effect-delay';
			break;
		case 'stack':
			$class = ' zp-effect-stackback zp-effect-delay';
			break;
		case '3D_flip':
			$class = 'zp-effect-3dflip zp-effect-delay';
			break;
		case 'bring_back':
			$class = 'zp-effect-bringback zp-effect-delay';
			break;
		case 'superscale':
			$class = 'zp-effect-superscale zp-effect-delay';
			break;
		case 'center_flip':
			$class = 'zp-effect-flip zp-effect-delay';
			break;
		case 'front_row':
			$class = 'zp-effect-frontrow zp-effect-delay';
			break;
	}
 ...
 */
function lmseo_related_portfolio() {
	global $post;
...
?>
	...
	<div class="related_container "><?php
				...
				<?php
										...
										$term_ids = array_values(wp_list_pluck($terms, 'parent'));
										$args = array(
											'post_type' => 'portfolio',
											'tax_query' => array(
												array(
	...
	<?php }
		...
		function lmseo_portfolio_shortcode($num_items, $type, $effect) {
		...
		function zp_custom_comment_number() {
			...
			if ($num_comments == 0) {
				$comments = __('No Comments', 'novo');
			} elseif ($num_comments > 1) {
				$comments = $num_comments . __(' Comments', 'novo');
			} else {
				$comments = __('1 Comment', 'novo');
			}
	...
	?>
		...
		/**
		 *  Returns Href, src, Title, alt, caption and description given attachment ID 
		 *
		 *  @param int $postid the post id
		 *  @param int/string $imagesize the image size 
		 */
		function lmseo_get_attachment($attachment, $imageSize = 'thumbnail') {

			//$attachment = get_post( $attachment_id );
			$imageAtt = wp_get_attachment_image_src($attachment->ID, $imageSize);
			return array(
				'source' => $imageAtt[0],
				'height' => $imageAtt[2],
				'width' => $imageAtt[1],
				...
				'src' => $attachment->guid,                 //original image source. 
		...
		<?php
				...
				if (!empty($attachments)) {
					...
					foreach ($attachments as $attachment) {
						$image = lmseo_get_attachment($attachment, $imagesize);
						$src = wp_get_attachment_image_src($attachment->ID, $imagesize);
						//$caption = $attachment->post_excerpt;
						...
						//All the information from the attached images
						//print_r(wp_get_attachment_metadata($attachment->ID));
						//print_r(lmseo_get_attachment($attachment,$imagesize));
					}
				...
				if ($().jPlayer) {
					...
					$('#jquery-jplayer-video-<?php echo $postid; ?>').bind($.jPlayer.event.playing, function(event) {
						$(this).add('#jp-video-interface-<?php echo $postid; ?>').hover(function() {
							$('#jp-video-interface-<?php echo $postid; ?>').stop().animate({
								opacity: 1
							}, 400);
						}, function() {
							$('#jp-video-interface-<?php echo $postid; ?>').stop().animate({
